...
namespace PMD.SaveEditor.Web.Services
{
    public class RBStoredPokemon
    {
        ...
        /* 
        // Logic for loading from file removed for web port, can be re-added if needed using byte[]
        ...
        */
        ...
        public override string ToString()
        {
            if (ID > 0)
            {
                var pkmName = Lists.RBPokemon.ContainsKey(ID) ? Lists.RBPokemon[ID] : "???";
                return string.Format(Language.SkyStoredPokemonToString, Name, Level, pkmName);
            }
            else
            {
                return Language.BlankPokemon;
            }
        }
    }
}
