        ...
        public override int GetByteCount(char[] chars, int index, int count)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                switch (chars[i])
                {
                    case '\0':
                        byteCount += 1;
                        break; // Fixed from return byteCount
                    case '\\':
            ...
            for (int i = 0; i < charCount; i++)
            {
                switch (chars[i])
                {
                    case '\0':
                        bytes[byteCount] = 0;
                        byteCount += 1;
                        break; // Fixed from return charCount
                    case '\\':
                        ...
                        break;
                    default:
                        var idx = eightBitChars.LastIndexOf(chars[i].ToString());
                        bytes[byteCount] = idx >= 0 ? (byte)idx : (byte)0x3F; // Use '?' if not found
                        byteCount += 1;
                        break;
                }
            }
            ...
            for (int i = index; i < count + index; i++)
            {
                switch (bytes[i])
                {
                    ...
                    case 0x87:
                        ...
                        {
                            charCount += 1; // Treat as single char if orphaned
                        }
                        break;
                    default:
                        charCount += 1;
                ...
                switch (current)
                {
                    ...
                    case 0x87:
                        if (i + 1 < byteCount)
                        {
                            ...
                            {
                                chars[charIndex + charCount + j] = escape[j];
                            }
                        ...
                        {
                            // Orphaned escape char
                            chars[charIndex + charCount] = '?';
                            charCount += 1;
                        }
                        break;
                    default:
                        chars[charIndex + charCount] = current < EightBitCharacters.Length ? (EightBitCharacters[current].Length > 0 ? EightBitCharacters[current][0] : '?') : '?';
                        charCount += 1;
                        break;
                }
            }
