...
namespace PMD.SaveEditor.Web.Services
{
    public class RBSave
    {
        ...
        public RBSave()
        {
            ...
            Bits = new BitBlock(0); // Initialize empty
        }

        public RBSave(byte[] rawData)
        {
            Offsets = new RBOffsets();
        ...
        public static bool IsOfType(byte[] file, int checksumEnd)
        {
             if (file.Length > checksumEnd)
             {
                // This checks if the file checksum matches.
                // Note: The original logic in IsOfType(GenericFile) was:
